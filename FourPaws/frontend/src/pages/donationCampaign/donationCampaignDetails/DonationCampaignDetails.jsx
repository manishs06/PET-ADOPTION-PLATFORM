...
const DonationCampaignDetails = () => {
    ...
    // Extract the campaign ID from the loaded data
    const campaignId = campdetails._id;

    // Use the correct property names from the API response
    const { image, title, description, endDate, targetAmount, currentAmount, currency, status } = campdetails;
    ...
    return (
        <div>
            <div className="relative flex flex-col justify-center items-center  lg:flex-row bg-clip-border mt-20 border-pink-700 border-2 p-10 mx-auto rounded-xl bg-white text-gray-700 shadow-md w-full max-w-[48rem] ">
                ...
                <div className="p-6 lg:border-l-2 border-none border-pink-700 ml-10">
                    <h6
                        ...
                        Campaign Ends: {new Date(endDate).toLocaleDateString()}
                    </h6>
                    ...
                    <div className="mb-4">
                        <div className="flex justify-between mb-1">
                            ...
                            <span className="text-sm font-medium text-blue-700">{currentAmount || 0} {currency} / {targetAmount} {currency}</span>
                        </div>
                    ...
                    <button 
                        ...
                        className={`btn ${status !== 'active' ? 'disabled:cursor-not-allowed bg-gray-500' : 'bg-pink-700'}`} 
                        onClick={() => document.getElementById('my_modal_5').showModal()}
                    >
                        {status === 'active' ? 'Donate Now' : 'Campaign Not Active'}
                        {status === 'active' && (
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                strokeWidth="2" className="w-4 h-4 ml-2">
                    ...
                    <dialog id="my_modal_5" className="modal modal-bottom sm:modal-middle">
                        <div className="modal-box">
                            <div>
                                <div className='text-center text-2xl font-bold mb-4'>Make a Donation</div>
                                <div>
                                    <Elements stripe={stripePromise}>
                                        {/* Pass the campaign ID as a prop to CheckoutForm */}
            ...
            <div className='flex flex-col items-center justify-center text-3xl font-bold mt-10'>
                ...
                <p className='text-xl bg-green-500 px-6 py-2 mt-4 rounded-md'>Donate More</p>
            </div>
